...
def _huggingface_response(
    ...
    if isinstance(datasets, Dataset):
        logger.info(f"Adding responses to dataset using model from {model_path}.")
    ...
    with torch.no_grad():
        ...
        if isinstance(datasets, Dataset):
            try:
                results: list = [
                    row[0]["generated_text"] for row in # type: ignore
                    pipe(datasets["chat"], batch_size=batch_size, max_new_tokens=max_length) # type: ignore
                ]
                datasets = datasets.add_column(
                    "chat_response", results
                ).remove_columns(
                    "chat"
                ).rename_column(
                ...
                logger.error("Something went wrong with generating or processing responses. Skipping..")
...
def _has_response(datasets: DatasetDict) -> bool:
    ...
    if isinstance(datasets, DatasetDict):
        ...
        return "chat" in datasets.column_names
    else:
        raise ValueError(f"Datasets should be of type Dataset or DatasetDict, not {type(datasets)}.")
    ...
    if isinstance(datasets, DatasetDict):
        ...
        return datasets[0]["chat"][-1]["role"] == "assistant"
    else:
        raise ValueError(f"Datasets should be of type Dataset or DatasetDict, not {type(datasets)}.")
